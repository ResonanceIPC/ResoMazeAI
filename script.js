...
function update_steps() {
    for (let y = 0; y < constant_height; y++) {
        for (let x = 0; x < constant_width; x++) {
            draw_round(y * block_size + block_size / 2, x * block_size + block_size / 2, block_size / 5, labyrinth[y][x][2])
        }
    }
}
...
function step_count_range_changer(coord) {
    let new_value = ((labyrinth[coord[0]][coord[1]][2] - 0) / (2 - 0)) * (10 - 0) + 0
    return new_value
}
...
function find_valid_neighbours_steps_rate(coord) {
    ...
    for (let i = 0; i < all_moves.length; i++) {
        ...
        if (all_moves[i][0] < coord[0] && labyrinth[coord[0]][coord[1]][0] == 1) {
            continue
        }
        if (all_moves[i][1] < coord[1] && labyrinth[coord[0]][coord[1]][1] == 1) {
            continue
        }
        if (all_moves[i][0] > coord[0] && labyrinth[coord[0] + 1][coord[1]][0] == 1) {
            continue
        }
        if (all_moves[i][1] > coord[1] && labyrinth[coord[0]][coord[1] + 1][1] == 1) {
            continue
        }
...
function find_best_move() {
    ...
    for (let i = 0; i < all_moves.length; i++) {
        ...
        if (all_moves[i][0] < player_y && labyrinth[player_y][player_x][0] == 1) {
            continue
        }
        if (all_moves[i][1] < player_x && labyrinth[player_y][player_x][1] == 1) {
            continue
        }
        if (all_moves[i][0] > player_y && labyrinth[player_y + 1][player_x][0] == 1) {
            continue
        }
        if (all_moves[i][1] > player_x && labyrinth[player_y][player_x + 1][1] == 1) {
            continue
        }
        if (labyrinth[all_moves[i][0]][all_moves[i][1]][2] >= 2) {
            continue
